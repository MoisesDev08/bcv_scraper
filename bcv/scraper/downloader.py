from bcv.scraper.exceptions import DownloadError, EmptyFileError, NotAExcelFile, HTMLInvalidWarning
...
class Downloader:
    def __init__(self, downloads_dir: str | Path, mkdir: bool = False, client: HttpClient = None):
        self.client = HttpClient() if not client else client
        self.downloads_dir = v.validate_downloads_dir(downloads_dir, mkdir=mkdir)
        self._downloads_hashes = []

    def _is_duplicate(self):
        # ABRE EL DOWNLOADS DIR, ITERA Y HACE HASHING
        ...
    ...
    def _links_scraper_generator(self, pages: int):
        ...
        for page_number in range(0, pages):
            ...
            try:
                if page_number == 0: page_number = None
                response = self.client.fetch(page=page_number)
                valid_html = v.validate_html(response)
                
                html = response.text
                scraper = LinksXLSFilesScraper(html=html)
                
            except HTMLInvalidWarning as e:

                logger.warning(f"Error parsing page N°{page_number if page_number else 'N/A'}")
            ...
            if not scraper.next_page_enabled():
                logger.info("Stopping scraper generator because of could not found 'Next Page' button")
                break
    ...
    def _choose_ext_helper(self, get_ext, headers_ext):
        if get_ext and headers_ext:
            if not get_ext == headers_ext: 
                ...
                logger.warning("Diferentes extensiones detectadas, usando la de la peticion GET...")
                if logger.isEnabledFor(logging.DEBUG):
    ...
    def _to_download_xls_and_save(
        ...
        try: r_head = self.client.fetch(url=link, head=True)
        except Exception: r_head = None
        
        if ext is None:
            get_ext, headers_ext = v.validate_excel_extension(r_get=r_get, r_head=r_head)
        ...
        if ext and isinstance(ext, str): name = name + ext
    ...
    def download(self, pages_quantity: int):
        ...
        scraper_generator: Iterable[LinksXLSFilesScraper] = self._links_scraper_generator(pages_quantity)
        for scraper in scraper_generator:
            ...
            for link_tuple in links_list:
                ...
                try:
                    ...
                    logger.critical("No extension detected, downloading without a extension...")
                    ext = ""
                    name = name + ext
