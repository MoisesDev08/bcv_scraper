    ...
    * *html (str):* the HTML from the 'Banco Central de Venezuela' oficial page
    ...
    def _validate_dolar_rate(self, value: str | float | int) -> float:
        ...
        if isinstance(value, str):
            ...
            if value < 0:
                raise HTMLParserError(f"Tasa no puede ser negativa: value=({value})")
            return float(value)

        else:
            raise HTMLParserError(
                "TypeError during rate validation", value=value, type_value=type(value)
            )
    ...
    def _validate_date_helper(self, value: Tag) -> datetime:
        ...
        if isinstance(fecha, list):
            raise HTMLParserError(
                "Unexpected HTML attribute value. Expected: str[ISO 8601 FORMAT date]",
                value_content_attr=fecha,
            )

        if not re.fullmatch(cf.ISOFORMAT_PATTERN, fecha):
            raise HTMLParserError(
        ...
        try:
            ...
            try:
                ...
                raise HTMLParserError(
                    "ValueError raised for datetime during date validation", fecha=fecha
                ) from e
    ...
    def fetch_data(self) -> tuple[datetime, float]:
        ...
        * *dolar_rate (float):* dolar rate in float obj"""
        ...
        if self.fecha_valor is None:
            self._get_dolar_date()
...
class LinksXLSFilesScraper:
    ...
    * *html (str):* the HTML from the 'Tipo de Cambio de Referencia (SMC)' page
    ...
    def _links_validator_helper(
        self, list_links_tags: dict[str, Tag]
    ) -> list[tuple[str, str]]:
        ...
        for k, tag in list_links_tags.items():
            ...
            if isinstance(href, str) and href.endswith(".xls"):
                ...
                if logger.isEnabledFor(logging.DEBUG) and href:
                    logger.debug(
                        f"Tag without link to .xls or bad formatted link:\n{href}"
                    )

                continue

        if logger.isEnabledFor(logging.DEBUG):
            logger.debug(f"{hf_err} errors during links validation")
        return valid_links
    ...
    def _get_links_helper(self, tr_tags: list[Tag]) -> dict:
        ...
        for tag in tr_tags:
            ...
            if title_tag is None:
                ...
                if logger.isEnabledFor(logging.DEBUG):
                    logger.debug(
                        f"Row tag without title element; {tag.prettify()[:150]}"
                    )
            ...
            if link_tag is None:
                ...
                if logger.isEnabledFor(logging.DEBUG):
                    logger.debug(
                        f"Row tag without link element; {tag.prettify()[:150]}"
                    )
    ...
    def get_links(self) -> list[tuple[str, str]]:
        ...
        if table_tag is None:
            raise SelectorNotFound(
                "Could not find table Tag element", selector=cf.TABLE_SELECTOR
            )
    ...
    def _next_page_enabled(self) -> bool:
        ...
        if len_tag == 0:
            logging.warning(
                "No 'Next Page' button were found. This might be the last page"
            )
            return False
